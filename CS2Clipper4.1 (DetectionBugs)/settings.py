...
class CS2ClipperSettings:
    ...
    def __init__(self, root):
        ...
        self.control_file = os.path.join(os.path.dirname(__file__), "process_control.txt")
    ...
    # END OF _create_footer - NO MORE CODE IN THIS METHOD
    
    # NEW METHOD STARTS HERE (same indent level as _create_footer)
    def on_closing(self):
        """Handle window close event (X button) - IMPROVED VERSION"""
        if self.process is not None and self.process.poll() is None:
            ...
            try:
                ...
                print(f"[WARNING] Could not write stop signal: {e}")
            
            # Give process more time to detect the stop signal and cleanup
            ...
            # Check for and cleanup any incomplete output files
            print("\nChecking for incomplete files...")
            incomplete_cleaned = self.cleanup_incomplete_output_files()
    ...
    # ANOTHER NEW METHOD (same indent level as on_closing)
    def cleanup_incomplete_output_files(self):
        """
        Search for and remove incomplete files in output folders.
        ...
        try:
            # Get folder paths from current settings
            output_folder = self.output_folder_var.get()
            unsorted_folder = self.unsorted_folder_var.get()
            
            folders_to_check = []
            if os.path.exists(output_folder):
                folders_to_check.append(output_folder)
            if os.path.exists(unsorted_folder):
                folders_to_check.append(unsorted_folder)
            
            # Patterns for incomplete files
            incomplete_patterns = [
                '*_trimmed.mp4',      # May be incomplete if just started writing
                '*_processed.mp4',    # May be incomplete
                '*_discord.mp4',      # May be incomplete from compression
            ]
            ...
            for folder in folders_to_check:
                for pattern in incomplete_patterns:
                    ...
                    for file_path in glob.glob(full_pattern):
                        ...
                        # (likely to be from the current session)
                        try:
                            file_age = time.time() - os.path.getmtime(file_path)
                            if file_age < 300:  # 5 minutes
                                # Additional check: file size is suspiciously small or zero
    ...
    def _create_processing_settings_section(self, main_frame, current_row):
        """Create the processing settings section (Num videos, Preview, Multiprocessing, Discord Compression)"""
        ...
        # === COMPRESSION SETTINGS BUTTON (AT BOTTOM) ===
        ttk.Button(
            processing_frame,
            text="Compression Settings",
            command=self.open_compression_settings,
            width=20
        ).grid(row=6, column=0, sticky=tk.W, pady=(10, 0))
    ...
    def open_compression_settings(self):
        """Placeholder for compression settings dialog"""
        messagebox.showinfo("Compression Settings", "Compression settings dialog - To be implemented")
