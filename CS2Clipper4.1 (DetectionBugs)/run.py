...
# Convert mode to boolean flags for backward compatibility
save_all_versions = (save_versions_mode == "Save All")
compress_for_discord = config.get("compress_for_discord", False)
...
def calculate_detection_thresholds(video_width, video_height):
    ...
    thresholds = {
        ...
        'DISAPPEAR_THRESHOLD': 180,  # Frame-based, no scaling
...
def process_single_video(args):
    ...
    try:
        ...
        # List of weapons to check (focusing on rifles for now)
        weapons_to_check = ['ak47', 'awp', 'm4a4', 'm4a1s']
                ...
                if w > thresholds['MIN_WIDTH'] and thresholds['MIN_HEIGHT'] < h < thresholds['MAX_HEIGHT']:
                    ...
                    if rect_area > thresholds['MIN_RECT_AREA']:
                        ...
                        if 0.05 < fill_ratio < 0.22:
                            ...
                            # Extract the region for additional checks
                            roi_bgr = killfeed_region[y:y+h, x:x+w]
                            roi_gray = cv2.cvtColor(roi_bgr, cv2.COLOR_BGR2GRAY)
                            
                            # Check 1: Minimum width (180px at 1080p, scaled for other resolutions)
                            min_width_threshold = int(round(180 * thresholds['width_scale']))
                            width_check = w >= min_width_threshold
                            
                            # Check 2: White pixel percentage (weapon icon indicator)
                            ...
                            white_check = white_ratio >= 0.03  # At least 3% white pixels
                            
                            # Only accept rectangles that pass BOTH checks
                            if width_check and white_check:
                                ...
                                # Optional: Show rejected rectangles in preview with reason
                                reason = []
                                if not width_check:
                                    reason.append(f"W:{w}px<{min_width_threshold}px")
                                if not white_check:
                                    reason.append(f"White:{white_ratio:.1%}<3%")
                                
                                # Draw rejected rectangles in orange
                                cv2.rectangle(killfeed_region, (x, y), (x+w, y+h), (0, 165, 255), 1)
                                cv2.putText(killfeed_region, f"REJECT: {', '.join(reason)}", 
                ...
                if is_new:
                    ...
                    for p_idx, (px, py, pw, ph, p_first_frame, p_count) in enumerate(pending_rectangles):
                        ...
                        if p_count + 1 >= thresholds['CONFIRMATION_FRAMES']:
                            ...
                            # Extract the red rectangle region for weapon detection
                            rect_region = killfeed_region[y:y+h, x:x+w]

                            DIAGNOSTIC_MODE = True
                            if DIAGNOSTIC_MODE:
                                all_scores = diagnose_detection_issue(rect_region, weapon_detector)

                            
                            # Detect weapon icon
                            weapon_name, weapon_confidence, debug_info = weapon_detector.detect_weapon_in_region(
                                rect_region, 
                                check_only=weapons_to_check
                            )
                            
                            # ========================================
                            # VISUAL DEBUG: Draw weapon detection on preview
                            # ========================================
                            if show_preview and worker_count == 1:
                                if weapon_name:
                                    # Draw GREEN rectangle around detected weapon icon
                                    cv2.rectangle(killfeed_region, (x, y), (x+w, y+h), (0, 255, 0), 3)
                                    
                                    # Add weapon name label above the rectangle
                                    label = f"{weapon_name} {weapon_confidence:.2f}"
                                    label_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)[0]
                                    
                                    # Background for text
                                    cv2.rectangle(killfeed_region, 
                                                (x, y - label_size[1] - 8), 
                                                (x + label_size[0] + 4, y - 2), 
                                                (0, 255, 0), -1)
                                    
                                    # Text
                                    cv2.putText(killfeed_region, label, 
                                            (x + 2, y - 5), 
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
                                else:
                                    # Draw YELLOW rectangle for unidentified weapon
                                    cv2.rectangle(killfeed_region, (x, y), (x+w, y+h), (0, 255, 255), 2)
                                    cv2.putText(killfeed_region, "Unknown", 
                                            (x + 2, y - 5), 
                            ...
                            if weapon_name:
                                print(f"  🎯 Weapon detected: {weapon_name} (confidence: {weapon_confidence:.3f})")
                            else:
                                print(f"  ❓ No weapon icon detected (best confidence: {weapon_confidence:.3f})")
                            
                            # Store in confirmed rectangles data
